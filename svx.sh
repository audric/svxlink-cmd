...
action_logs() {
    ...
    if [[ "$source" == "journald" ]]; then
        journalctl -u "${svc}.service" -f 2>/dev/null || true
    else
...
main_menu() {
    ...
    local menu_items=(
        ...
        "8"  "$(printf "%-${W}s" "Edit GPIO config")"
        "9"  "$(printf "%-${W}s" "Edit environment defaults")"
        "─3" "$(printf "%-${W}s" "─── Audio ──────────────")"
        "10" "$(printf "%-${W}s" "Alsamixer")"
        "─4" "$(printf "%-${W}s" "─── Boot & GPIO ────────")"
        "11" "$(printf "%-${W}s" "Enable service at boot")"
        "12" "$(printf "%-${W}s" "Disable service at boot")"
        "13" "$(printf "%-${W}s" "Restart GPIO setup")"
    )
